...
/**
  ...
  * @dev Tru Reputation Protocol Sale contract based on Open Zeppelin and 
...
contract TruSale is Ownable, Haltable {
     ...
     * @param _purchaser Purchaser who paid for the tokens
     * @param _recipient Recipient who received the tokens
     * @param _weiValue Amount raised in wei used in the purchase
    ...
    */
    event WhiteListUpdate(address indexed _purchaserAddress, bool _whitelistStatus);

    /**
     * @notice Sale End Time Changed Event
     * @param _newEnd New time the Sale ends at
    */
    event EndChanged(uint256 _oldEnd, uint256 _newEnd);
    ...
    // @dev Function to add or disable a purchaser from AML Whitelist
    ...
    // @dev Function to change the end time of the Sale
    ...
    // @dev Function to validate that the buy is occuring within the Sale window and before the Cap is reached
    // @returns bool Returns true if the buy meets the criteria, false if it does not 
    ...
    // @dev Haltable purchase validation function. Performs all pre-checks before processing purchase
    ...
    // @dev Internal function used to encapsulate more complex constructor logic and ensure
    ...
    // @dev Private function used to execute the purchase of Tokens in this sale
